...
class TestBlockToMiddle(unittest.TestCase):
    ...
    def test_block_to_middle_with_correction(self):
        """方向転換後に角度補正を行う場合のテスト."""
        angle = -45
        have_block = True  # ブロックを保持している
        can_correction = True
        ...
        # コストの期待値を求める
        motion_time = 0.8285 + \
            GameMotion.ROTATION_BLOCK_TABLE[45]["time"] + GameMotion.SLEEP_TIME * 3
        success_rate = 0.62
        expected_cost = motion_time*success_rate+GameMotion.MAX_TIME*(1-success_rate)
    ...
    def test_block_to_middle_zero_angle_with_correction(self):
        """方向転せずに角度補正を行う場合のテスト."""
        angle = 0
        have_block = True  # ブロックを保持している
        can_correction = True
        ...
        # コストの期待値を求める
        motion_time = 0.8285 + \
            GameMotion.ROTATION_BLOCK_TABLE[0]["time"] + GameMotion.SLEEP_TIME * 2
        success_rate = 0.62
        expected_cost = motion_time*success_rate+GameMotion.MAX_TIME*(1-success_rate)
