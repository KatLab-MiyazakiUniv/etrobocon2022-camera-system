...
class ColorChanger:
    ...
    def change_color(self, path, save_path) -> None:
        """画像を6色画像に変換する関数.
        ...
        # 赤抽出
        mask1 = cv2.inRange(hsv, ColorChanger.__LOWER[0], ColorChanger.__UPPER[0])
        mask2 = cv2.inRange(hsv, ColorChanger.__LOWER[1], ColorChanger.__UPPER[1])
        frame_mask.append(mask1 + mask2)

        # 黄、緑、青、白抽出
        for i in range(2, 6):
            frame_mask.append(cv2.inRange(hsv, ColorChanger.__LOWER[i], ColorChanger.__UPPER[i]))

        for i in range(len(frame_mask)):
            # 論理演算で色検出（検出しなかった部分は黒）
            frame_mask[i] = cv2.bitwise_and(img, img, mask=frame_mask[i])
            # 色の置換
